...
function initWorldGen(game) {
  game.world.on('worldDataNeeded', function(id, data, x, y, z) {
    worldGenQueue.push({
      id: id, 
      data: data,
      x:x, y:y, z:z
  ...
  game.on('tick', function() {
    processWorldGenQueue(game)
  })
}

var worldGenQueue = []

function processWorldGenQueue(game) {
  if (worldGenQueue.length===0) return
  var obj = worldGenQueue.shift()
  generateWorld( obj.data, obj.x, obj.y, obj.z )
  game.world.setChunkData( obj.id, obj.data )
}
