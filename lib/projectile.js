...
function makeProjectileLauncher(game, particleAdder) {
  ...
  return function(spelltype, size, gravMult, friction, restitution, option) {
    ...
    game.entities.addComponent(eid, game.entities.names.collideTerrain)

    // timer for bomb
    if (spelltype===2) {
      game.entities.addComponent(eid, game.entities.names.every, {
        every: 3000,
        callback: function() {
          var pos = game.entities.getPositionData(eid).position
          addBlocksInSphere(game, 0, pos, 2.75)
          game.entities.getState(eid, game.entities.names.particles).parts.rate = 0
          // add smoke
          var parts = particleAdder('bombsmoke')
          parts.mesh.position.copyFromFloats( pos[0], pos[1]+0.5, pos[2] )
          
          game.entities.deleteEntity(eid)
        }
      })
    }
    ...
    if (spelltype===1) {
      var dat = game.entities.getState(eid, game.entities.names.collideTerrain)
    ...
    game.entities.addComponent(eid, game.entities.names.particles, { parts: parts })
...
function onCollideTerrain(game, eid, option) {
  ...
  game.entities.removeComponent(eid, game.entities.names.collideTerrain)
